...
def compute_similarities_for_layer(layer_name, layer_idx, embs, nsd_ids, chunk_size=500, n_sample_images=10000, sample_size=500000):
    ...
    # Across-image: sample images to keep memory manageable
    ...
    # Take first caption per image — preserves per-image variance, no averaging collapse
...
fig.suptitle('Cosine Similarity Distributions by Layer\n(sentence-transformers/all-mpnet-base-v2)', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig(output_dir / 'similarity_distributions_sentence_transformers.png', dpi=150, bbox_inches='tight')
print(f"Saved plot to {output_dir / 'similarity_distributions_sentence_transformers.png'}")
